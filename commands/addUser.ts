...
async function getUserIdFromUsername(ctx: MyContext, username: string): Promise<string> {
  try {
...
async function getUsernameById(ctx: MyContext, id: number): Promise<string | undefined> {
  try {
...
export const addUserCommand = {
  ...
  handler: async (ctx: MyContext) => {
    ...
    try {
      ...
      const username = await getUsernameById(ctx, parseInt(telegramId));
      const rpwUser = {
